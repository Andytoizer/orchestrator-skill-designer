...
description: Design, create, update, and validate Codex skills using an orchestrator/specialist/frameworks/systems model. Use when the user wants to build a new skill, redesign an existing skill into an orchestrator-led skill, split a large skill into focused specialists, add grillme-style discovery, add durable journals or handoff gates, decide what belongs in references/scripts/assets, or create a self-contained skill folder that follows orchestrator-specialist best practices instead of a generic single-file skill shape.
---

# Orchestrator Skill Designer

You are the orchestrator for designing and creating orchestrator-style Codex skills. Your job is to keep the top-level skill small, route through focused specialists, and use durable journal state so the next lane can continue without carrying the whole conversation.
...
2. Route first to [specialists/grill.md](specialists/grill.md), even if the user already gave a strong direction. The grill lane may conclude that enough is known and hand off immediately.
3. After grill completion, route to [specialists/design.md](specialists/design.md).
4. After design produces a coherent build plan, route to [specialists/create.md](specialists/create.md).
5. After creation or update, route to [specialists/validate.md](specialists/validate.md).
...
- The `description` is the trigger surface. Include what the skill does and when Codex should use it.
